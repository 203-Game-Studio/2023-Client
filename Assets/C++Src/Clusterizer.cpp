...
namespace meshopt
{
	...
	static void computeBoundingBox(float result[6], const float points[][3], size_t count)
	{
		assert(count > 0);

		// find extremum points along all 3 axes; for each axis we get a pair of points with min/max coordinates
		float pmin[3] = { FLT_MAX, FLT_MAX, FLT_MAX };
		float pmax[3] = { FLT_MIN, FLT_MIN, FLT_MIN };
		...
		for (size_t i = 0; i < count; ++i)
		{
			...
			for (int axis = 0; axis < 3; ++axis)
			{
				pmin[axis] = std::min(p[axis], pmin[axis]);
				pmax[axis] = std::max(p[axis], pmax[axis]);
			}
		}
...
size_t meshopt_buildMeshlets(meshopt_Meshlet* meshlets, unsigned int* meshlet_vertices, unsigned char* meshlet_triangles, const unsigned int* indices, size_t index_count, const float* vertex_positions, size_t vertex_count, size_t vertex_positions_stride, size_t max_vertices, size_t max_triangles, float cone_weight)
{
	...
	for (size_t i = 0; i < meshlet_offset; ++i)
	{
		const meshopt_Meshlet& m = meshlets[i];
		auto bound = meshopt_computeMeshletBounds(&meshlet_vertices[m.vertex_offset], &meshlet_triangles[m.triangle_offset],
			m.triangle_count, vertex_positions, vertex_count, vertex_positions_stride);
		memcpy(meshlets[i].min, bound.min, 3 * sizeof(float));
		memcpy(meshlets[i].max, bound.max, 3 * sizeof(float));
		memcpy(meshlets[i].cone_apex, bound.cone_apex, 3 * sizeof(float));
		memcpy(meshlets[i].cone_axis, bound.cone_axis, 3 * sizeof(float));
		meshlets[i].cone_cutoff = bound.cone_cutoff;
	}
...
meshopt_Bounds meshopt_computeClusterBounds(const unsigned int* indices, size_t index_count, const float* vertex_positions, size_t vertex_count, size_t vertex_positions_stride)
{
	...
	// fill bounding box info; note that below we can return bounds without cone information for degenerate cones
	float boundingBox[6] = {};
	computeBoundingBox(boundingBox, corners[0], triangles * 3);
	bounds.min[0] = boundingBox[0];
	bounds.min[1] = boundingBox[1];
	bounds.min[2] = boundingBox[2];
	bounds.max[0] = boundingBox[3];
	bounds.max[1] = boundingBox[4];
	bounds.max[2] = boundingBox[5];
